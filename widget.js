...
(function () {
    ...
    function initConfig() {
        ...
        if (script) {
            ...
            config.apiUrl =
                script.getAttribute("data-api-url") || config.apiUrl;
        }
    ...
    function addMessage(
        ...
        messageDiv.className = `gulp-message ${
            isUser ? "gulp-message-user" : "gulp-message-assistant"
        }`;
    ...
    async function sendQuery(queryText) {
        ...
        try {
            ...
            const response = await fetch(
                `${config.apiUrl}/api/v1/widget/query`,
                {
                    method: "POST",
                    headers: {
